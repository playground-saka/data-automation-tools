...
const menuList: MenuItem[] = [
  {
    id: 0,
    label: "Notification",
    icon: <BellIcon className="h-4 w-4 text-zinc-800/80 group-hover:text-zinc-800" />,
  ...
  },
  {
    id: 1,
    label: "Dashboard",
    icon: <PresentationChartBarIcon className="h-4 w-4 text-zinc-800/80 group-hover:text-zinc-800" />,
    hasDropdown: false,
    link: "/dashboard",
  },
  {
    id: 2,
    label: "Master",
    icon: <BuildingOffice2Icon className="h-4 w-4 text-zinc-800/80 group-hover:text-zinc-800" />,
    hasDropdown: true,
    dropdownItems: [
  ...
  },
  {
    id: 3,
    label: "Analisis Data",
    icon: <ChartBarSquareIcon className="h-4 w-4 text-zinc-800/80 group-hover:text-zinc-800" />,
    hasDropdown: true,
    dropdownItems: [
      { label: "Selisih", link: "/analisis-data/selisih" },
    ],
    link: "/analisis-data"
  },
];
  ...
  return (
    <aside className="bg-white min-h-screen h-screen w-full max-w-[18rem] border-r flex flex-col fixed top-0">
      <div className="px-8 py-6 flex flex-row items-center gap-2 pt-12">
        <h1 className='text-2xl font-bold'>Daman.<span className='text-xs text-stone-800/30'>V1.0.0</span></h1>
      </div>
        ...
        <div className='flex flex-row justify-between items-center px-4'>
          <div className='flex flex-row items-center gap-4 px-4 py-4'>
            ...
            <div className='flex flex-col'>
              <h1 className='text-xs text-stone-800'>Irvan Gerhana S.</h1>
              <h1 className='text-xs text-stone-800/70'>Chief Technology Officer</h1>
            </div>
          </div>
        </div>
      </div>
            ...
            {item.hasDropdown ? (
              ...
              >
                {isActive(item.link, item.dropdownItems) && (
                  <span
                    className="absolute -left-0.5 top-1/4 h-1/2 w-1 rounded-full bg-indigo-500"
                  ></span>
                )}
                <div className='flex flex-row gap-2 items-center'>
                  <div className='mr-2'>
                    {React.cloneElement(item.icon, {
                      className: `h-5 w-5 ${
                        isActive(item.link, item.dropdownItems) ? 'text-indigo-500 group-hover:text-indigo-500' : 'text-zinc-800/70 group-hover:text-zinc-800'
                      }`,
                    })}
                  </div>
                  <p className={`text-sm ${isActive(item.link, item.dropdownItems) ? 'text-indigo-500 group-hover:text-indigo-500' : 'text-zinc-800/70 group-hover:text-zinc-800'}`}>
                    {item.label}
                  </p>
                </div>
                <ChevronDownIcon
                  strokeWidth={2.5}
                  className={`h-4 w-4 transition-transform ${open === item.id ? "rotate-180" : ""} ${isActive(item.link, item.dropdownItems) ? 'text-indigo-500' : ''}`}
                />
              ...
              >
                ...
                <div className='flex flex-row gap-2 items-center'>
                  <div className='mr-2'>
                    {React.cloneElement(item.icon, {
                      className: `h-5 w-5 ${
                        isActive(item.link) ? 'text-indigo-500 group-hover:text-indigo-500' : 'text-zinc-800/70 group-hover:text-zinc-800'
                      }`,
                    })}
                  </div>
                  <p className={`text-sm ${isActive(item.link) ? 'text-indigo-500 group-hover:text-indigo-500' : 'text-zinc-800/70 group-hover:text-zinc-800'}`}>
                    {item.label}
                  </p>
                </div>
                {item.notificationCount && (
              ...
              <div className="flex flex-col gap-1">
                {item.dropdownItems.map((dropdownItem, index) => (
                  <Link key={`${item.id}-${index}`} href={dropdownItem.link}>
                    <div className={`w-full flex flex-row items-center justify-between gap-2 px-4 py-2.5 cursor-pointer transition-all ease-in-out hover:bg-gray-100 group ${isActive(dropdownItem.link) ? 'font-bold' : ''}`}>
